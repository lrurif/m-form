...
function deepFind(value, list, res) {
  for (const item of list) {
    res.push(item.label);
    if (item.value === value) {
      return res;
    }
    const data = (item.children && deepFind(value, item.children, res));
    if (data) {
      return data;
    }
    res.pop();
...
const chooseText = computed(() => {
  return deepFind(modelValue.value, props.setApis.attrs?.data || [], [])?.join?.('/');
});
</script>
...
<template>
  <el-form-item v-bind="setApis.attrs" :prop="dataSource.alias || dataSource.uuid" :rules="rules">
    ...
    <el-cascader
      v-if="formType !== '4'"
      v-bind="setApis.attrs" v-model="modelValue" :class="[setApis.attrs.isFill ? 'width-fill' : '']" :options="setApis.attrs?.data || []"
      :props="{
        emitPath: false,
