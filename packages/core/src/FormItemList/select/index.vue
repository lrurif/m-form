...
const chooseText = computed(() => {
  return props.setApis.attrs?.data?.find(item => item.value === modelValue.value)?.label;
});
</script>
...
<template>
  <el-form-item v-bind="setApis.attrs" :prop="dataSource.alias || dataSource.uuid" :rules="rules">
    ...
    >
      <el-option v-for="item in setApis.attrs?.data || []" :key="item.value" :value="item.value" :label="item.label" />
    </el-select>
